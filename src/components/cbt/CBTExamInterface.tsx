...
const CBTExamInterface: React.FC<CBTExamInterfaceProps> = ({ test, attempt, onSubmit, onExit }) => {
  ...
  // Always points at the CURRENT handleAutoSubmit. The timer effect below has
  // an empty dep array, so it captured render-0's callback — whose `answers`
  // was still the empty initial map. A trainee who answered every question and
  // let the clock reach 00:00 had their attempt submitted blank and scored 0.
  // Only the timeout path was affected, which made it look intermittent.
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setTimeRemaining(prev => {
        if (prev <= 1) {
          clearInterval(timer);
          // Deferred: submitting here would run the parent's setState during
          // this updater, i.e. during render.
  ...
  const handleAutoSubmit = useCallback(() => {
    // flaggedForReview was omitted here but included in confirmSubmit, so a
    // timed-out attempt also lost the trainee's flagged questions.
  ...
  }, [test, attempt, answers, tabSwitchCount, flaggedQuestions, onSubmit]);
  ...
  // Truthiness, not `!== null`: a question id missing from the answers map
  // (a resumed attempt whose stored ids drifted) is undefined, and
  // `undefined !== null` would mark it as answered.
  const isQuestionAnswered = (questionId: string) => Boolean(answers[questionId]);
  const getAnsweredCount = () => test.questions.filter(q => isQuestionAnswered(q.id)).length;
  ...
  return (
    <div ref={examRef} className="fixed inset-0 bg-gray-100 z-50 flex flex-col" style={{ userSelect: 'none', WebkitUserSelect: 'none' }}>
      ...
      {/* Header */}
      {/* Header — the title is truncated and the controls are pinned, so a long
          test title can no longer push the timer and Submit button off a phone
          screen. The answered count is now visible at every width; it is the
          number a candidate checks most before submitting. */}
      <div className="bg-white border-b shadow-sm px-3 sm:px-4 py-2 sm:py-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
            ...
            {tabSwitchCount > 0 && (
              <span className="hidden sm:inline-block px-2 py-1 bg-red-100 text-red-700 text-xs font-medium rounded shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            {/* Timer */}
            <div
              className={`flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg font-mono text-base sm:text-lg font-bold tabular-nums ${
                isTimeCritical
            ...
            {/* Progress */}
            <div className="flex items-baseline gap-1 text-xs sm:text-sm text-gray-600 tabular-nums">
              <span className="font-medium text-gray-800">{getAnsweredCount()}</span>
              <span>/</span>
            ...
            <button
              ...
              className="px-3 sm:px-4 py-1.5 sm:py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg text-sm sm:text-base font-medium transition-colors flex items-center gap-2 shrink-0"
            >
              <svg className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
