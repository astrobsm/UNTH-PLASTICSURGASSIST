...
type AdminTab = 'dashboard' | 'user-approvals' | 'users' | 'bulk-import' | 'team-analytics' | 'roles' | 'rotations' | 'system' | 'database' | 'security' | 'analytics' | 'settings';
...
export default function Admin() {
  const [activeTab, setActiveTab] = useState<AdminTab>(() => {
    ...
    return (tab && ['dashboard','user-approvals','users','bulk-import','team-analytics','roles','rotations','system','database','security','analytics','settings'].includes(tab))
      ? tab as AdminTab : 'dashboard';
  });

  // Student management now lives in Training Admin, alongside the trainees it
  // is scored against. Anything still pointing at /admin?tab=students -- a
  // bookmark, an old sidebar link -- is forwarded rather than shown an empty tab.
  const navigate = useNavigate();
  useEffect(() => {
    if (new URLSearchParams(window.location.search).get('tab') === 'students') {
      navigate('/training-admin?tab=students', { replace: true });
    }
  }, [navigate]);
  ...
  if (createdCredentials) {
    return createPortal(
      <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[200] p-4 overflow-y-auto">
        <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-auto my-auto max-h-[90vh] overflow-y-auto">
          <div className="p-6">
