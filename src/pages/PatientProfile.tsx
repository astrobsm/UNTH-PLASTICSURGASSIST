...
const PatientWoundMonitor = lazy(() => import('../pages/WoundProgressMonitorPage').then(m => ({ default: m.PatientWoundMonitor })));
...
import {
  ...
  Eye, Trash2, Upload, Mic, MicOff, ScanLine, RefreshCw, Download
} from 'lucide-react';
...
export const PatientProfile: React.FC = () => {
  ...
  const renderSection = (sectionId: string) => {
    switch (sectionId) {
      ...
      case 'wound-monitor':
        // The monitor, not a second wound form. It carries a wound's identity
        // between visits, so serial assessments describe one wound healing
        // rather than six unrelated snapshots.
        return (
          <Suspense fallback={<div className="h-32 bg-gray-50 rounded animate-pulse" />}>
            <PatientWoundMonitor patient={patient} />
          </Suspense>
        );
      case 'fluid-io':
