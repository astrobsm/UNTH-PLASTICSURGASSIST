...
import { extendRotation, signOutRotation, evaluateDueRotations, ROTATION_ADMIN_ROLES } from './_lib/rotationLifecycle.js';
...
async function handleGet(req, res, userId, userRole) {
  ...
  switch (action) {
    ...
    case 'all':
      ...
      if (!ROTATION_ADMIN_ROLES.includes(userRole)) {
...
async function handlePut(req, res, userId, userRole) {
  ...
  switch (action) {
    case 'extend': {
      // Shared with Training Admin through _lib/rotationLifecycle.js. The two
      // used to hold separate copies that disagreed about who may extend and
      ...
      const result = await extendRotation({
        rotationId, days: extensionDays, reason, byUserId: userId,
      });
      if (!result.ok) return res.status(400).json({ error: result.error });
      return res.status(200).json({ rotation: result.rotation });
    }
    ...
    case 'approve-signout': {
      ...
      const out = await signOutRotation({
        rotationId, mode: 'approved', reason: comments, byUserId: userId, finalScore,
      });
    ...
    case 'evaluate': {
      // Every rotation past its end date: signed out where the score allows,
      // flagged for a decision where it does not. A trainee may run it for
      // themselves; only a supervisor may sweep everyone.
      const mine = !ROTATION_ADMIN_ROLES.includes(userRole);
      const result = await evaluateDueRotations(mine ? { userId } : {});
