 ...
 * 
 * This creates ALL tables needed by the Plastic Surgery Assistant app.
 * Uses SERIAL (integer) IDs consistently since that's what the API layer expects.
 */
...
async function runMigration() {
  console.log('🚀 Starting comprehensive migration to NEW database...\n');

  // ===== Step 1: Core Tables from init-db.js =====
  ...
  const coreTables = `
    ...
    -- Treatment Plan Modifications table
    ...
    -- Surgery Bookings table (extended surgery tracking)
    CREATE TABLE IF NOT EXISTS surgery_bookings (
      ...
      assistant_surgeon_id INTEGER REFERENCES users(id),
      anaesthetist_id INTEGER REFERENCES users(id),
      scrub_nurse_id INTEGER REFERENCES users(id),
    ...
    CREATE TABLE IF NOT EXISTS surgical_checklists (
      id SERIAL PRIMARY KEY,
      surgery_booking_id INTEGER NOT NULL REFERENCES surgery_bookings(id) ON DELETE CASCADE,
      sign_in_completed BOOLEAN DEFAULT FALSE,
      sign_in_completed_by INTEGER REFERENCES users(id),
      ...
      time_out_completed_by INTEGER REFERENCES users(id),
      ...
      sign_out_completed_by INTEGER REFERENCES users(id),
    ...
    -- Lab Investigations table (extended lab tracking)
  ...
  await query(coreTables);
  console.log('  ✅ Core tables created\n');

  // ===== Step 2: Training & Education Tables =====
  console.log('📋 Step 2: Creating training & education tables...');
  ...
  const trainingTables = `
    ...
    CREATE TABLE IF NOT EXISTS mcq_questions (
      id SERIAL PRIMARY KEY,
      cme_topic_id INTEGER REFERENCES cme_topics(id) ON DELETE CASCADE,
    ...
    CREATE TABLE IF NOT EXISTS user_assessments (
      ...
      cme_topic_id INTEGER NOT NULL REFERENCES cme_topics(id) ON DELETE CASCADE,
  ...
  await query(trainingTables);
  console.log('  ✅ Training & education tables created\n');

  // ===== Step 3: Clinical Assessment Tables =====
  console.log('📋 Step 3: Creating clinical assessment tables...');
  ...
  const clinicalTables = `
    ...
    -- Diabetic Foot Assessments table
    ...
    -- Pressure Sore Assessments table
  ...
  await query(clinicalTables);
  console.log('  ✅ Clinical assessment tables created\n');

  // ===== Step 4: Procedures & Checklists =====
  console.log('📋 Step 4: Creating procedures & checklists tables...');
  ...
  await query(procedureTables);
  console.log('  ✅ Procedures & checklists tables created\n');

  // ===== Step 5: Ward & Discharge Tables =====
  console.log('📋 Step 5: Creating ward & discharge tables...');
  ...
  await query(wardTables);
  console.log('  ✅ Ward & discharge tables created\n');

  // ===== Step 6: MDT & Team Tables =====
  console.log('📋 Step 6: Creating MDT & team tables...');

  const mdtTables = `
    -- MDT Patient Teams table
    ...
    CREATE TABLE IF NOT EXISTS mdt_cases (
      id SERIAL PRIMARY KEY,
      mdt_meeting_id INTEGER NOT NULL REFERENCES mdt_meetings(id) ON DELETE CASCADE,
      patient_id INTEGER NOT NULL REFERENCES patients(id) ON DELETE CASCADE,
      case_summary TEXT NOT NULL,
      presenting_clinician_id INTEGER REFERENCES users(id),
      discussion_points TEXT,
  ...
  await query(mdtTables);
  console.log('  ✅ MDT & team tables created\n');

  // ===== Step 7: Keloid Care Tables =====
  console.log('📋 Step 7: Creating keloid care tables...');

  const keloidTables = `
    -- Keloid Care Plans table
    ...
    -- Keloid Pretreatment Tests table
  ...
  await query(keloidTables);
  console.log('  ✅ Keloid care tables created\n');

  // ===== Step 8: Consumables Tables =====
  console.log('📋 Step 8: Creating consumables tables...');

  const consumablesTables = `
    -- Surgical Consumables table
    ...
    CREATE TABLE IF NOT EXISTS consumable_usage (
      id SERIAL PRIMARY KEY,
      consumable_id INTEGER NOT NULL REFERENCES surgical_consumables(id),
      patient_id INTEGER REFERENCES patients(id) ON DELETE SET NULL,
      surgery_booking_id INTEGER REFERENCES surgery_bookings(id) ON DELETE SET NULL,
  ...
  await query(consumablesTables);
  console.log('  ✅ Consumables tables created\n');

  // ===== Step 9: Chat & Conference Tables =====
  console.log('📋 Step 9: Creating chat & conference tables...');
  ...
  await query(chatTables);
  console.log('  ✅ Chat & conference tables created\n');

  // ===== Step 10: System Tables =====
  console.log('📋 Step 10: Creating system tables...');
  ...
  const indexes = `
    ...
    CREATE INDEX IF NOT EXISTS idx_patients_app_id ON patients(app_id);
  ...
  console.log('📋 Step 12: Creating admin users...');

  // Create douglas@unth.ng user (the main user)
  const douglasHash = await bcrypt.hash('Surgeon@2026', 10);
  try {
    await query(
      `INSERT INTO users (username, password_hash, email, full_name, role, is_approved, is_active, must_change_password, app_id)
       VALUES ($1, $2, $3, $4, $5, true, true, false, 'psa')
       ON CONFLICT (email) DO NOTHING`,
      ['douglas', douglasHash, 'douglas@unth.ng', 'Dr. Douglas', 'admin', ]
    );
    console.log('  ✅ User douglas@unth.ng created (password: Surgeon@2026, role: admin)');
  } catch (e) {
    console.log('  ℹ️ User douglas@unth.ng may already exist:', e.message);
  }
  ...
  try {
    await query(
      `INSERT INTO users (username, password_hash, email, full_name, role, is_approved, is_active, must_change_password, app_id)
       VALUES ($1, $2, $3, $4, $5, true, true, false, 'psa')
       ON CONFLICT (email) DO NOTHING`,
      ['admin', adminHash, 'admin@hospital.com', 'System Administrator', 'admin']
    );
    console.log('  ✅ User admin@hospital.com created (password: Admin@123!, role: admin)');
  } catch (e) {
    console.log('  ℹ️ User admin@hospital.com may already exist:', e.message);
  }

  console.log('');

  // ===== Step 13: Verify =====
  console.log('📋 Step 13: Verifying migration...');
  ...
  console.log('✅ ====================================');
  console.log('\n📌 Next steps:');
  console.log('   1. Update Vercel DATABASE_URL environment variable');
  console.log('   2. Redeploy to Vercel');
  console.log('   3. Test login at the app URL');
}
